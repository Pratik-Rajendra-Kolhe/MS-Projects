...
public class ThreadedIndexerRunner {
	...
	protected ThreadedIndexerRunner(Properties idxProps) throws ClassNotFoundException, IOException {
		this.props = idxProps;
	...
	private class RunnerThread implements Runnable {
		...
		private RunnerThread(int pnum) throws ClassNotFoundException, IOException {
				...
				if (etr == null) {
					...
					try {
						writer.addToIndex(etr.term, etr.docId,
								etr.numOccurances);
					} catch (IndexerException | ClassNotFoundException | IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
				}
