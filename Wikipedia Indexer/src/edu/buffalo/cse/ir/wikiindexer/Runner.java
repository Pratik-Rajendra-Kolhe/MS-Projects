...
public class Runner {
	 ...
	 * @throws ClassNotFoundException 
	 */
	private static void runIndexer(Properties properties) throws ClassNotFoundException, IOException {
	...
	private static void tokenizeAndIndex(Properties properties,
			ConcurrentLinkedQueue<WikipediaDocument> queue) throws InterruptedException, ClassNotFoundException, IOException {
		/*
		 * Pseudo-code:
		...
		try {
			while (remaining > 0) {
				...
				idoc = pool.take().get();
//				System.out.println(idoc.getStream(INDEXFIELD.AUTHOR).getAllTokens().toString());
//				System.out.println(idoc.getStream(INDEXFIELD.TERM).getAllTokens().toString());
//				System.out.println(idoc.getStream(INDEXFIELD.LINK).getAllTokens().toString());
//				System.out.println(idoc.getStream(INDEXFIELD.CATEGORY).getAllTokens().toString());
				if (idoc != null) {
					currDocId = docDict.lookup(idoc.getDocumentIdentifier());
//					System.out.println("DOC-ID "+idoc.getDocumentIdentifier());
					TokenStream stream;
					try {
					...
					//		System.out.println(stream.getAllTokens().toString());
							...
							if (stream != null) {
                             ...
                             //   System.out.println(tokenmap.entrySet().toString());
								if (tokenmap != null) {
									switch (fld) {
									case TERM:
