...
%calculating the hypothesis values
for i=1:1500
    for j=1:10
       ak(j)=exp(x_test(i,1:513)*w(:,j));
    end
     y_test(i,:)=ak/sum(ak);
     ak(1:10)=0;
end
...
error_rate=(1500-sum(num))/1500;
 ...
 err=0;  %cross entropy eror function
    for i=1:1500
        cnt2=0;
        for k=1:10
            cnt2=cnt2+t_test(i,k)*log2(y_test(i,k));
        end
        err=err+cnt2;
    end    

  err=-1*err;
  cross_entropy_err_test=err/19978;
